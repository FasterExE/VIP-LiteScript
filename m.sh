...
cek_reg_mode() {
    until [[ $MYIP =~ ^[0-9.]+$ && ${CLIENT_EXISTS} == '1' ]]; do
        ...
        CLIENT_EXISTS=$(wget -qO- https://raw.githubusercontent.com/FasterExE/VIP-LiteScript/ip | grep -w $MYIP | wc -l)
...
[Unit]
Description=Ilyass Server Xray
...
function install_slowdns(){
    print_install "Ilyass SlowDNS Server"
...
print_ok "Selesai pemasangan modul Ilyass"
}
